...
def _row(label: str, value: str, *, href: str = "", suffix: str = "") -> str:
    shown = f'<a href="{escape(href, quote=True)}">{escape(value)}</a>' if href else escape(value)
...
<a href="https://github.com/schlomo/ipp-joblog">ipp-joblog</a>
...
class Overview:
    ...
    def card(printer: PrinterSummary) -> str:
        ...
        admin = (
            f'<span class="ui"><a href="{escape(web_ui, quote=True)}">Printer web UI ↗</a></span>'
            if web_ui
            else ""
        )
        return (
...
updated {self.generated_at.strftime("%d %b %Y %H:%M:%S")}</p>
{self.cards()}
<footer>Generated by <a href="https://github.com/schlomo/ipp-joblog">ipp-joblog</a>
